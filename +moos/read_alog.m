% function [x] = read_async_log(filename,DataPath, matFile)
...
filename = '/home/rgcofield/gps_class/grp/data/Novatel_Data_16_9_2013_____13_30_04___cleaned.alog';
matFile  = '/home/rgcofield/gps_class/grp/data/Novatel_Data.mat';
tic
        ...
        if msg_idx >-1 || isempty(input(dvc_idx,2))
            ...
            if ~isempty(input{dvc_idx,msg_idx})
                %cell not empty--> expand length
                 ...
                 if length( value )==1
                     moos.close_up_gaps(dvc_idx, length(input(dvc_idx,:)), length(time_data(dvc_idx,2).time));
                 end
                ...
                %Close Up gaps in non-cell data
                if length( value )==1
                    moos.close_up_gaps(dvc_idx, msg_idx, length(time_data(dvc_idx,2).time));
                end
...
%                 data (dvc_idx,msg_idx).time = time(1);
             ...
             %Close Up gaps in non-cell data
             if length( value )==1
                 moos.close_up_gaps(dvc_idx, data_msg_idx, length(time_data(dvc_idx,2).time));
             end
