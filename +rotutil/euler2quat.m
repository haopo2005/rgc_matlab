...
%    disp('order is x-->y-->z');
    Q1=rotutil.quaternionmul(Qy,Qz);  
    Q2=rotutil.quaternionmul(Qx,Q1);  
    
elseif(strcmpi(order,'xzy')==1)  
 %   disp('order is x-->z-->y');
    Q1=rotutil.quaternionmul(Qz,Qy);  
    Q2=rotutil.quaternionmul(Qx,Q1);
    
elseif(strcmpi(order,'yxz')==1)
  %  disp('order is y-->x-->z');
    Q1=rotutil.quaternionmul(Qx,Qz);  
    Q2=rotutil.quaternionmul(Qy,Q1);
    
elseif(strcmpi(order,'yzx')==1)
   % disp('order is y-->z-->x');
    Q1=rotutil.quaternionmul(Qz,Qx);  
    Q2=rotutil.quaternionmul(Qy,Q1);
    
elseif(strcmpi(order,'zxy')==1)
    %disp('order is z-->x-->y');
    Q1=rotutil.quaternionmul(Qx,Qy);  
    Q2=rotutil.quaternionmul(Qz,Q1);
    
elseif(strcmpi(order,'zyx')==1) % same as matlab quatdemo
    %disp('order is z-->y-->x');
    Q1=rotutil.quaternionmul(Qy,Qx);  
    Q2=rotutil.quaternionmul(Qz,Q1); 
else
